...
-- Copiando estrutura do banco de dados para arquivos
DROP DATABASE IF EXISTS `arquivos`;
CREATE DATABASE IF NOT EXISTS `arquivos` /*!40100 DEFAULT CHARACTER SET utf8 */;
USE `arquivos`;

-- Copiando estrutura para tabela arquivos.empresa
...
-- Copiando estrutura para tabela arquivos.pedido
...
-- Copiando estrutura para tabela arquivos.produtos
DROP TABLE IF EXISTS `produtos`;
CREATE TABLE IF NOT EXISTS `produtos` (
  `id_produtos` int(11) NOT NULL AUTO_INCREMENT,
  `descricao` varchar(50) DEFAULT NULL,
  `precoVenda` double DEFAULT NULL,
  `estoque1` double DEFAULT NULL,
  `estoque2` double DEFAULT NULL,
...
CREATE ALGORITHM=TEMPTABLE DEFINER=`mysql.sys`@`localhost` SQL SECURITY INVOKER VIEW `memory_global_total` AS SELECT sys.format_bytes(SUM(CURRENT_NUMBER_OF_BYTES_USED)) total_allocated FROM performance_schema.memory_summary_global_by_event_name ;
...
CREATE ALGORITHM=MERGE DEFINER=`mysql.sys`@`localhost` SQL SECURITY INVOKER VIEW `ps_check_lost_instrumentation` AS SELECT variable_name, variable_value FROM performance_schema.global_status WHERE variable_name LIKE 'perf%lost' AND variable_value > 0 ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`mysql.sys`@`localhost` SQL SECURITY INVOKER VIEW `session` AS SELECT * FROM sys.processlist WHERE conn_id IS NOT NULL AND command != 'Daemon' ;
